...
rm -rf $LOGFILES/*
rm -rf $PREFIX/$TARGET/lib/libSDL* $PREFIX/$TARGET/lib/libogg* $PREFIX/$TARGET/lib/libvorbis* $PREFIX/$TARGET/lib/libfreetype* $PREFIX/bin/sdl-config $PREFIX/$TARGET/bin/freetype-config
rm -rf $BUILDS/*
...
cp -rf $WORKSPACE/_install/recipes/files.wd/SDL/video/amigaos/* $BUILDS/build-$LIBSDL_APOLLO_NAME/video/amigaos >>$LOGFILES/part10_sdl_prepare.log 2>>$LOGFILES/part10_sdl_prepare_err.log
echo -e -n "make | "
make -j1 >>$LOGFILES/part10_sdl_make.log 2>>$LOGFILES/part10_sdl_make_err.log
echo -e "install\e[0m"
